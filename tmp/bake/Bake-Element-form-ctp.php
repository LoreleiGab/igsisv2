...
<nav class="col-md-2 columns" id="actions-sidebar">
    <ul class="nav nav-pills nav-stacked">
        <li class="active"><a><CakePHPBakeOpenTag= __('Ação') CakePHPBakeCloseTag></a></li>
<?php if (strpos($action, 'add') === false): ?>
        <li><CakePHPBakeOpenTag= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $<?= $singularVar ?>-><?= $primaryKey[0] ?>],
                ['confirm' => __('Você tem certeza que deseja excluir # {0}?', $<?= $singularVar ?>-><?= $primaryKey[0] ?>)]
            )
...
?>
        <li><CakePHPBakeOpenTag= $this->Html->link(__('Listar {0}', '<?= $this->_pluralHumanName($alias) ?>'), ['controller' => '<?= $details['controller'] ?>', 'action' => 'index']) ?></li>
        <li><CakePHPBakeOpenTag= $this->Html->link(__('Novo {0}', '<?= $this->_singularHumanName($alias) ?>'), ['controller' => '<?= $details['controller'] ?>', 'action' => 'add']) ?></li>
<?php
...
?>
    ...
    <CakePHPBakeOpenTag= $this->Form->button(__('Gravar')) CakePHPBakeCloseTag>
